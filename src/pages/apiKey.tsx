...
import { Data } from "./api/createApi";
...
const Page = () => {
  ...
  const userInsert = useMutation(api.yt.userInsert);

  const [copyNotification, setCopyNotification] = useState(false);

  async function handleApiGen() {
    if (!user) {
      alert("You need to be logged in to create an api key");
      return;
    }
    const res = await fetch("/api/createApi", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="flex justify-center flex-col h-screen items-center">
      {apiKey ? (
          ...
          <div className="relative z-20 flex items-center">
            ...
            >
              {!copyNotification ? (
                ...
                <>
                  <span className="tracking-tight text-green-500" x-cloak>
                    Copied to Clipboard
                  </span>
                  <svg
        ...
        <button
          className="border-green-600 border-2 p-2 rounded-md"
          type="button"
          onClick={handleApiGen}
        >
          Create a api key
        </button>
      )}
